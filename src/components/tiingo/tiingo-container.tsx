...
import { 
  ...
  ChevronRight, ArrowUpRight, Activity, Loader2, AlertCircle, Link2
} from "lucide-react";
import { clsx } from "clsx";

type TiingoEndpoint = "daily" | "daily_meta" | "news" | "crypto" | "forex" | "iex" | "fundamentals" | "dividends" | "splits" | "search";
...
const API_STRUCTURE: APIConfig[] = [
  { label: "2.1 End-of-Day", endpoint: "daily", icon: BarChart2, url: "https://api.tiingo.com/tiingo/daily/<ticker>/prices" },
  { label: "2.1 Meta Data", endpoint: "daily_meta", icon: Search, url: "https://api.tiingo.com/tiingo/daily/<ticker>" },
  { label: "2.2 News", endpoint: "news", icon: Newspaper, url: "https://api.tiingo.com/tiingo/news" },
  { label: "2.3 Crypto", endpoint: "crypto", icon: Coins, url: "https://api.tiingo.com/tiingo/crypto/prices" },
  { label: "2.4 Forex", endpoint: "forex", icon: Zap, url: "https://api.tiingo.com/tiingo/fx/<ticker>/top" },
  { label: "2.5 IEX", endpoint: "iex", icon: Landmark, url: "https://api.tiingo.com/iex/<ticker>" },
  { label: "2.6 Fundamentals", endpoint: "fundamentals", icon: Activity, url: "https://api.tiingo.com/tiingo/fundamentals/<ticker>/daily" },
  { label: "2.8 Dividends", endpoint: "dividends", icon: ArrowUpRight, url: "https://api.tiingo.com/tiingo/corporate-actions/<ticker>/distributions" },
  { label: "2.9 Splits", endpoint: "splits", icon: ArrowUpRight, url: "https://api.tiingo.com/tiingo/corporate-actions/<ticker>/splits" },
  { label: "4.1 Search", endpoint: "search", icon: Search, url: "https://api.tiingo.com/tiingo/utilities/search?query=<query>" },
];
    ...
    <div className="flex h-[calc(100vh-64px)] bg-[#f8f9fc]">
      ...
      <aside className="w-80 bg-white border-r border-slate-200 overflow-y-auto flex flex-col shadow-sm">
        <div className="p-6 border-b border-slate-50">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Endpoints Documentation</p>
        </div>

        <nav className="flex-1 overflow-y-auto p-4 space-y-2">
          {API_STRUCTURE.map((item) => (
            <button 
              key={item.endpoint}
        ...
        <header className="flex justify-between items-end mb-8 border-b border-slate-100 pb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="bg-blue-100 text-blue-600 text-[10px] font-black px-2 py-0.5 rounded uppercase">REST Endpoint</span>
              <span className="text-slate-300 text-[10px] font-bold">●</span>
              <span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest">Mock Enabled</span>
            </div>
            <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tighter italic leading-none">
              {activeEndpoint.replace('_', ' ')} <span className="text-blue-600">Terminal</span>
            </h2>
          </div>
