...
import { 
  ...
  ChevronRight, ArrowUpRight, Activity, Loader2, AlertCircle, Link2, Wifi, Globe, Database, Tag, Layers
} from "lucide-react";
...
interface APIConfig {
  ...
  url: string;
  icon: any;
  category: "Series" | "Category" | "Release" | "Source" | "Tag" | "WebSocket" | "REST" | "Utility";
}
...
const FRED_STRUCTURE: APIConfig[] = [
  // V1 Series
  { category: "Series", label: "Series Info", endpoint: "series", icon: Activity, url: "https://api.stlouisfed.org/fred/series?series_id=<id>" },
  { category: "Series", label: "Observations", endpoint: "observations", icon: BarChart2, url: "https://api.stlouisfed.org/fred/series/observations?series_id=<id>" },
  { category: "Series", label: "Updates", endpoint: "series_updates", icon: RefreshCcw, url: "https://api.stlouisfed.org/fred/series/updates" },
  
  // V1 Category
  { category: "Category", label: "Category Info", endpoint: "category", icon: Layers, url: "https://api.stlouisfed.org/fred/category?category_id=0" },
  { category: "Category", label: "Children", endpoint: "category_children", icon: ChevronRight, url: "https://api.stlouisfed.org/fred/category/children?category_id=0" },
  
  // V1 Release
  { category: "Release", label: "All Releases", endpoint: "releases", icon: Globe, url: "https://api.stlouisfed.org/fred/releases" },
  { category: "Release", label: "Release Info", endpoint: "release", icon: Info, url: "https://api.stlouisfed.org/fred/release?release_id=10" },
  { category: "Release", label: "Release Obs", endpoint: "release_observations", icon: Landmark, url: "https://api.stlouisfed.org/fred/release/observations?release_id=10" },
  
  // V1 Source
  { category: "Source", label: "All Sources", endpoint: "sources", icon: Database, url: "https://api.stlouisfed.org/fred/sources" },
  { category: "Source", label: "Source Info", endpoint: "source", icon: Landmark, url: "https://api.stlouisfed.org/fred/source?source_id=1" },
  
  // V1 Tags
  { category: "Tag", label: "All Tags", endpoint: "tags", icon: Tag, url: "https://api.stlouisfed.org/fred/tags" },
  { category: "Tag", label: "Tag Series", endpoint: "tag_series", icon: Search, url: "https://api.stlouisfed.org/fred/tags/series?tag_names=monetary+policy" },
];
...
export default function TiingoContainer() {
  ...
  const categories = Array.from(new Set(currentStructure.map(a => a.category)));
  ...
  const handleProviderChange = (p: ApiProvider) => {
    setProvider(p);
    const firstEndpoint = p === "tiingo" ? "daily" : "series";
      ...
      <aside className="w-80 bg-white border-r border-slate-200 overflow-y-auto flex flex-col shadow-sm">
        <div className="p-6 border-b border-slate-50 space-y-4">
          ...
          <div className="flex bg-slate-100 p-1 rounded-xl">
            ...
            <button onClick={() => handleProviderChange("fred")} className={clsx("flex-1 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all", provider === "fred" ? "bg-white shadow-sm text-red-600" : "text-slate-400")}>FRED V1/V2</button>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-6">
          {categories.map((cat) => (
            <div key={cat} className="space-y-2">
              ...
              {currentStructure.filter(a => a.category === cat).map((item) => (
                <button key={item.endpoint} onClick={() => setActiveEndpoint(item.endpoint)}
                  ...
                  <div className={clsx("text-[9px] font-mono break-all opacity-60", activeEndpoint === item.endpoint ? "text-blue-50" : "text-slate-400")}>
                    {item.url.replace('<ticker>', searchSymbol).replace('<id>', searchSymbol).replace('<query>', searchSymbol)}
                  </div>
                </button>
              ))}
            </div>
        ...
        <header className="flex justify-between items-end mb-8 border-b pb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="bg-blue-100 text-blue-600 text-[10px] font-black px-2 py-0.5 rounded uppercase">{provider} Center</span>
              <span className="text-slate-400 text-[10px] font-bold uppercase tracking-widest italic ml-2">Protocol Sandbox</span>
            </div>
          ...
          <div className="flex gap-2">
            <div className="relative">
              ...
              <input type="text" value={searchSymbol} onChange={(e) => setSearchSymbol(e.target.value.toUpperCase())} className="bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-3 text-sm font-bold focus:outline-none focus:ring-4 focus:ring-blue-50 w-48 shadow-sm" placeholder="Ticker / ID" />
            </div>
            <button onClick={() => refetch()} className="bg-slate-900 text-white px-8 py-3 rounded-xl text-sm font-black uppercase tracking-wider hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 italic">Test API</button>
          </div>
        </header>
